...
export async function handleCreateQuest(
  ...
  const newQuest = await env.DB.prepare(
    "INSERT INTO quest (adventure_id, title, create_time, description, deleted) VALUES (?, ?, ?, ?, FALSE) RETURNING quest_id",
  )
...
export async function handleDeleteQuest(
  ...
  const result = await env.DB.prepare(
    `UPDATE quest SET deleted=TRUE WHERE quest_id = ? AND deleted = FALSE`,
  )
    .bind(parseInt(request.params.id))
    .run();
  if (!result.meta.rows_written) {
